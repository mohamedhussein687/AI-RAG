...
def decide_payload(message, allowed=True):
...
def test_decide_returns_database_query_for_waiting_projects(client, auth_headers):
    response = client.post("/api/agent/decide", headers=auth_headers, json=decide_payload("كم مشروع waiting؟"))
